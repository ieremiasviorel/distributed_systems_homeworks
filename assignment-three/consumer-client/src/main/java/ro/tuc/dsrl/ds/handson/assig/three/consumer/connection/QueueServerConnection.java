 ...
 *               Provides one method which allows for requests to be sent to the
 *               server. Requests are in form of Message. Please refer to
 *               Message class for further details.
 */
public class QueueServerConnection {
	...
	/**
	 * Retrieves a message from the queue of the server, by sending a "READ" request
	 * to it. If the queue is empty, this method will hang until the queue will get
	 * a message. In other words, this method will wait for the server to provide a
	 * message.
	 *
	...
	public String readDVDMessage() throws IOException {
		Socket clientSocket = new Socket(host, port);
		ObjectOutputStream outToServer = new ObjectOutputStream(clientSocket.getOutputStream());
		ObjectInputStream inFromServer = new ObjectInputStream(clientSocket.getInputStream());
